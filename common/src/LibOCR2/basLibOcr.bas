...
' DEPENDENCIES                                              '
'                                                           '
' basLibVBA, CallbackT, DAOConnectionT, ProcessesT,         '
' ProcessInfoT, OcrConvertT                                 '
'                                                           '
' NOTES                                                     '
'                                                           '
...
Public Const kOcrQueryProcesses As String = "qryProcesses"
Public Const kOcrQueryProcessOcrData As String = "qryProcessOcrData"
...
Public Function ocrStart( _
    ...
    Dim processes As New ProcessesT
...
Private Function ocrCatalogFile( _
    ...
    aProcessInfo As ProcessInfoT, _
    aProcessRules As ProcessOcrDataT, _
    aOcrConvert As OcrConvertT, _
...
Private Function ocrExtractData( _
    ...
    If aProcessRules.Count > 0 Then
        ...
        While Not aProcessRules.EOF
            ...
            ' Only keep data where the rule's storage field isn't empty.
            If Not (IsEmpty(ocrValue) Or Nz(aProcessRules.StorageField) = "") Then _
                aData.PushBack NewPairT(aProcessRules.StorageField, ocrValue)
...
Private Function ocrGetCatalog( _
    ...
    '
    Select Case ocrStorageMethod(aProcessInfo.SaveToTable, aProcessInfo.SaveToProcedure, aProcessInfo.SaveToField)
        Case osmBuiltIn:
...
Private Function ocrProcessStart( _
    aProcess As ProcessesT, _
    aOcrConvert As OcrConvertT, _
    Optional aCallback As CallbackT = Nothing, _
    Optional ByVal aOnError As VbMsgBoxResult = vbIgnore _
) As Long
    Dim connection As DAOConnectionT, procinfo As New ProcessInfoT, rules As New ProcessOcrDataT, _
...
Private Function ocrStoreData( _
    ...
    fileData.PushBack NewPairT(aProcessInfo.SaveToField, aDestinationPath)
