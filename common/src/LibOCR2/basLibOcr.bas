...
Try:
    ...
    ocrText = ocrFileConvert(sourcePath, aOcrConvert)
...
Private Function ocrExtractData( _
    ...
    '
    ' Appends aData with a PairT containing aProcessRules.StorageField
    ' and corresponding data extracted from aOcrText, if any:
    '   ("ID",0123456789).
    ' Returns a list of required data extracted from the text as an
    ' array of Variant().
    '
...
Private Function ocrGetValue( _
    ...
    '
    ' Searches for and returns data extracted from the given text, if any.
    '
    Dim regex As Object
...
Private Function ocrStorageDelegate( _
    ...
    '
    ' Calls the storage delegate according to the aCatalog object type.
    '
...
Public Function NewOcrParamsT( _
    Optional aOcrParams As Variant, _
    Optional aTimeout As Long = -1, _
    Optional aWindowStyle As VbAppWinStyle = 0 _
) As OcrParamsT
    Dim obj As New OcrParamsT
    
    If IsObject(aOcrParams) Then
        Set obj.OcrParams = aOcrParams
    Else
        obj.OcrParams = aOcrParams
    End If
    obj.Timeout = aTimeout
    obj.WindowStyle = aWindowStyle
    Set NewOcrParamsT = obj
    Set obj = Nothing
End Function
...
Public Function NewOcrConvertT( _
    ...
    obj.Init aCallback, aOcrConvert
