...
Private Sub FormConnectionOpen( _
    ...
    '
    ' Closes the current connection and opens the given database connection.
    '
    FormConnectionClose
    With NewDAOConnectionT(aConnection)
        Set dbCurrent = OpenDatabase(.Database, IIf(.Name = "ODBC", False, Nothing), False, .connect)
    End With
End Sub

Public Sub FormConnectionReset()
    '
    ' Set the current connection to nothing.
    '
...
Public Sub FormDeleteRecord2( _
    aForm As Form _
)
    '
    ' Deletes the current record from the underlying recordset
    ...
    With aForm
        If Not aForm.NewRecord Then
            ...
            rst.Bookmark = .Bookmark
            rst.MovePrevious
            If Not rst.BOF Then i = rst.AbsolutePosition
            .Recordset.Delete
            .Requery
            If i <> 0 Then .Recordset.Move (i - 1)
        End If
    End With
    Set rst = Nothing
End Sub
        ...
        Select Case CInt(Args(0))   ' args(0) is the "action", args(1) is the "parameter".
            Case dlgAddNew: ' Action: add new record.
                ...
                If UBound(Args) > 0 Then
                    ...
                    aForm.Controls(vals(0)) = vals(1)   ' Assign "value" to from control "key"
                End If
...
Private Sub Misc()
    Dim cc As ConversionControlsT
    
    Set cc = NewConversionControlsT( _
        NewPropertyControlT(Nothing, "Name", 42, "") _
    )
    Debug.Print cc.Controls.Size
    Set cc = Nothing
End Sub
